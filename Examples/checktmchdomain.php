...
function checkdomains($conn, $domains) {
    try {
        ...
        if ($response = $conn->request($check)) {
            /* @var $response Guanjia\EPP\eppLaunchCheckResponse */
                ...
                if ($check['claimed']) {
                    if ($check['claim']) {
                        if ($check['claim'] instanceof eppDomainClaim) {
                            ...
                            throw new Guanjia\EPP\eppException("Domain name " . $check['domainname'] . " is claimed, but no valid claim key is present");
                        }

                    } else {
                        throw new Guanjia\EPP\eppException("Domain name " . $check['domainname'] . " is claimed, but no claim key is present");
                    }
